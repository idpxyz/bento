...
class OutboxSettings(BaseSettings):
    ...
    dispatch_interval_seconds: int = Field(2, ge=1, le=60, description="Interval between outbox dispatches")
    batch_size: int = Field(50, ge=1, le=1000, description="Maximum number of events to process per batch")
...
class AppSettings(BaseSettings):
    ...
    timezone: str = "UTC"
    debug: bool = False
    secret_key: SecretStr = Field(default="dev-secret-key-for-local-testing-only-32chars", min_length=32, description="Secret key for cryptographic operations")
