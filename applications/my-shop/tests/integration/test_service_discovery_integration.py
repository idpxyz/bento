...
async def test_service_not_found_error(monkeypatch):
    ...
    monkeypatch.setenv("SERVICE_DISCOVERY_BACKEND", "env")
    # Must set at least one service URL for env backend to work
    monkeypatch.setenv("SERVICE_CATALOG_SERVICE_URL", "http://catalog:8001")
...
async def test_kubernetes_backend_configuration(monkeypatch):
    ...
    # Kubernetes backend still requires env URLs as fallback
    monkeypatch.setenv("SERVICE_CATALOG_SERVICE_URL", "http://catalog:8001")
    monkeypatch.setenv("SERVICE_ORDER_SERVICE_URL", "http://order:8002")
    ...
    try:
        ...
        # When using ENV backend (fallback), host is parsed from URL
        # Kubernetes DNS format would be: <service>.<namespace>.svc.cluster.local
        assert instance.service_name == "catalog-service"
        assert instance.host == "catalog"  # From SERVICE_CATALOG_SERVICE_URL
        assert instance.port == 8001
    finally:
        # Cleanup: run shutdown phases
        await runtime._lifecycle_manager.shutdown()
