...
class CreateOrderItemUseCase:
    ...
    def __init__(self, repository, unit_of_work):
        """
        参数：
            repository: IOrderItemRepository - 仓储协议实例
            unit_of_work: IUnitOfWork - 工作单元协议实例
        """
        self._repository = repository
        self._uow = unit_of_work

    async def validate(self, command: CreateOrderItemCommand) -> None:
        """验证命令（可选）
    ...
    async def execute(self, command: CreateOrderItemCommand) -> str:
        """执行用例
        ...
        async with self._uow:
            ...
            #     email=command.email
            # )
            # await self._repository.save(orderitem)
...
#
# class CreateOrderItemUseCase(BaseUseCase[CreateOrderItemCommand, ID]):
...
#     """
#
#     async def handle(self, command: CreateOrderItemCommand) -> ID:
#         repo = self.uow.repository(OrderItem)
#         orderitem = OrderItem(...)
