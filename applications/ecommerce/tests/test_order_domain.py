...
def test_add_item_to_order():
    ...
    order.add_item(
        product_id=ID.generate(), product_name="iPhone 15 Pro", quantity=2, unit_price=999.99
    )
...
def test_pay_order():
    ...
    order = Order(order_id=ID.generate(), customer_id=ID.generate())
    order.add_item(product_id=ID.generate(), product_name="Product", quantity=1, unit_price=100.0)
...
def test_order_to_dict():
    ...
    order = Order(order_id=ID.generate(), customer_id=ID.generate())
    order.add_item(product_id=ID.generate(), product_name="Product A", quantity=2, unit_price=50.0)
