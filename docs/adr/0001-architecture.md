# ADR-0001: 六边形架构 + DDD 分层

## 状态
已采纳

## 上下文

在构建 Bento DDD 框架时，我们需要一个清晰的架构模式来：
- 将业务逻辑与技术实现解耦
- 支持多种接口类型（HTTP, gRPC, GraphQL, CLI）
- 便于测试和替换基础设施组件
- 保持领域模型的纯粹性

## 决策

我们采用**六边形架构**（Hexagonal Architecture，也称端口与适配器模式）结合 **DDD 战术设计**。

## 六边形架构核心原则

### 1. 架构概览

```
┌─────────────────────────────────────────────────────────────┐
│                        外层：适配器                          │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐         │
│  │  HTTP API   │  │   gRPC      │  │  GraphQL    │  驱动侧 │
│  └──────┬──────┘  └──────┬──────┘  └──────┬──────┘ (Primary)│
│         │                │                │                 │
│         └────────────────┼────────────────┘                 │
│                          ▼                                  │
│  ┌───────────────────────────────────────────────────┐     │
│  │            应用层 (Application)                   │     │
│  │  ┌─────────────┐  ┌──────────┐  ┌─────────────┐  │     │
│  │  │  Use Cases  │  │   UoW    │  │    Saga     │  │     │
│  │  └─────────────┘  └──────────┘  └─────────────┘  │     │
│  └───────────────────┬───────────────────────────────┘     │
│                      ▼                                      │
│  ┌───────────────────────────────────────────────────┐     │
│  │            领域层 (Domain)                        │     │
│  │  ┌──────────┐  ┌──────────┐  ┌──────────────┐    │     │
│  │  │ Entity   │  │VO/Event  │  │ DomainService│    │     │
│  │  └──────────┘  └──────────┘  └──────────────┘    │     │
│  └───────────────────┬───────────────────────────────┘     │
│                      ▼                                      │
│  ┌─────────────────────────────────────────────────────┐   │
│  │          基础设施层 (Infrastructure)                │   │
│  │  端口 ←→ 适配器                                    │   │
│  ├─────────────────────────────────────────────────────┤   │
│  │ Repository Port → SQLAlchemy / InMemory            │   │
│  │ EventBus Port   → Pulsar / Redis / InMemory        │   │
│  │ Cache Port      → Redis / Memcached                │   │
│  │ Storage Port    → MinIO / S3 / FileSystem          │   │
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
│                                          被驱动侧(Secondary)│
└─────────────────────────────────────────────────────────────┘
```

### 2. 分层职责

#### 🎯 Core（核心层）
**路径**: `src/core/`  
**职责**: 提供框架级别的基础抽象，不依赖任何其他层
- `Result<T,E>`: 函数式错误处理
- `EntityId`: 统一的实体标识
- `Guard`: 前置条件检查
- `Clock`: 时间抽象

**原则**:
- ✅ 零依赖（除了 Python 标准库）
- ✅ 纯函数，无副作用
- ❌ 不能导入其他业务层

#### 🎯 Domain（领域层）
**路径**: `src/domain/`  
**职责**: 封装核心业务逻辑和规则
- `Entity`: 有唯一标识的领域对象
- `ValueObject`: 不可变的值对象
- `AggregateRoot`: 聚合根，事务边界
- `DomainEvent`: 领域事件
- `Repository (Protocol)`: 仓储接口（端口）
- `Specification`: 业务规则封装

**原则**:
- ✅ 只依赖 `core`
- ✅ 纯业务逻辑，与技术无关
- ❌ 不能有任何 I/O 操作
- ❌ 不能依赖框架（FastAPI, SQLAlchemy 等）

#### 🎯 Application（应用层）
**路径**: `src/application/`  
**职责**: 编排用例，协调领域对象
- `UseCase`: 用例入口（命令/查询）
- `UnitOfWork`: 工作单元模式
- `Saga`: 分布式事务编排
- `DTO`: 数据传输对象

**原则**:
- ✅ 依赖 `core` + `domain` 的端口
- ✅ 通过依赖注入获取基础设施实现
- ❌ 不直接依赖具体的适配器实现

#### 🎯 Infrastructure（基础设施层）
**路径**: `src/infrastructure/`  
**职责**: 实现领域层定义的端口
- `cache.py`: 缓存适配器
- `storage.py`: 对象存储适配器
- `emailer.py`: 邮件发送适配器
- `locker.py`: 分布式锁适配器

**原则**:
- ✅ 实现 `domain` 中定义的 Protocol
- ✅ 可以使用第三方库（Redis, MinIO 等）
- ✅ 可替换（通过依赖注入切换实现）

#### 🎯 Persistence（持久化层）
**路径**: `src/persistence/`  
**职责**: 数据持久化的具体实现
- `InMemoryRepository`: 内存仓储（测试用）
- `SQLAlchemyRepository`: SQL 仓储
- `OutboxRepository`: Outbox 模式实现

#### 🎯 Messaging（消息层）
**路径**: `src/messaging/`  
**职责**: 事件驱动架构支持
- `EventBus (Protocol)`: 事件总线端口
- `Outbox (Protocol)`: 发件箱模式端口
- `codec.py`: 消息序列化

#### 🎯 Interfaces（接口层）
**路径**: `src/interfaces/`  
**职责**: 对外提供的驱动适配器
- `http.py`: REST API（FastAPI）
- `grpc.py`: gRPC 服务
- `graphql.py`: GraphQL API
- `scheduler.py`: 定时任务

**原则**:
- ✅ 调用 `application` 层的 UseCase
- ✅ 处理协议转换（HTTP → DTO）
- ❌ 不直接调用 `domain` 层

### 3. 依赖规则（洋葱模型）

```
┌──────────────────────────────────────┐
│      Interfaces (HTTP/gRPC)          │ ─┐
├──────────────────────────────────────┤  │
│      Infrastructure (SQLAlchemy)     │  │  外层依赖内层
├──────────────────────────────────────┤  │
│      Application (UseCases)          │  │
├──────────────────────────────────────┤  │
│      Domain (Entities/Events)        │  │
├──────────────────────────────────────┤  │
│      Core (Result/Guard)             │ ─┘
└──────────────────────────────────────┘

依赖方向：单向向内（外层 → 内层）
```

**强制执行**（通过 import-linter）:
```python
# pyproject.toml
[[tool.importlinter.contracts]]
name = "Hexagonal layering"
type = "layers"
layers = [
  "core",           # 最内层
  "domain",
  "application",
  "infrastructure",
  "interfaces",     # 最外层
]
```

### 4. 端口与适配器模式

#### 端口（Ports）
使用 Python `Protocol` 定义契约：

```python
# domain/repository.py - 端口定义
from typing import Protocol, Generic, TypeVar

T = TypeVar("T")

class Repository(Protocol, Generic[T]):
    """仓储端口 - 由基础设施层实现"""
    async def get(self, id: EntityId) -> Optional[T]: ...
    async def save(self, entity: T) -> None: ...
```

#### 适配器（Adapters）
实现端口的具体技术方案：

**驱动侧适配器**（外部 → 应用）:
- HTTP Adapter (`interfaces/http.py`)
- gRPC Adapter (`interfaces/grpc.py`)
- CLI Adapter (`toolkit/cli.py`)

**被驱动侧适配器**（应用 → 外部）:
- SQLAlchemy Repository (`persistence/sqlalchemy/`)
- Pulsar EventBus (`messaging/pulsar_bus.py`) - 优先
- Kafka EventBus (`messaging/kafka_bus.py`) - 可选
- Redis Cache (`infrastructure/redis_cache.py`)

### 5. 领域事件 + Outbox 模式

#### 为什么需要 Outbox？
解决**分布式事务一致性**问题：
- 数据库事务提交 ✅
- 事件发送失败 ❌ ← 导致不一致

#### Outbox 工作流程

```
1. 业务操作
   ┌─────────────────┐
   │  AggregateRoot  │
   │  创建/修改实体  │
   │  record_event() │ ← 收集领域事件
   └────────┬────────┘
            ▼
2. 事务内写入
   ┌──────────────────────────┐
   │   UnitOfWork.commit()    │
   ├──────────────────────────┤
   │ BEGIN TRANSACTION        │
   │   UPDATE entity_table    │
   │   INSERT INTO outbox     │ ← 同一事务
   │ COMMIT                   │
   └────────┬─────────────────┘
            ▼
3. 异步发布（独立进程）
   ┌──────────────────────────┐
   │  OutboxPublisher (Job)   │
   │  SELECT * FROM outbox    │
   │  WHERE published = false │
   │   ↓                      │
   │  Publish to Pulsar       │
   │   ↓                      │
   │  UPDATE outbox           │
   │  SET published = true    │
   └──────────────────────────┘
```

#### 实现示例

```python
# persistence/sqlalchemy/uow_sqlalchemy.py
class SQLAlchemyUnitOfWork:
    async def commit(self) -> None:
        # 1. 收集聚合的领域事件
        events = []
        for aggregate in self._tracked_aggregates:
            events.extend(aggregate.collect_events())
        
        # 2. 同一事务内写入 Outbox
        for event in events:
            outbox_record = OutboxMessage(
                topic=event.name,
                payload=event.to_dict(),
                created_at=now_utc(),
            )
            self.session.add(outbox_record)
        
        # 3. 提交事务（原子性保证）
        await self.session.commit()
```

### 6. 测试策略

#### 单元测试（Domain）
```python
# 纯领域逻辑，无需 Mock
def test_order_cannot_exceed_limit():
    order = Order.create(items=[...])
    assert order.total_amount <= Money(10000, "USD")
```

#### 集成测试（Application）
```python
# 使用 InMemory 适配器
async def test_create_order_usecase():
    repo = InMemoryOrderRepository()
    uow = InMemoryUnitOfWork()
    usecase = CreateOrder(repo, uow)
    
    result = await usecase(CreateOrderInput(...))
    assert result.is_ok
```

#### E2E 测试（Interfaces）
```python
# 完整的 HTTP → DB 流程
async def test_order_api_flow(client: TestClient):
    response = await client.post("/api/orders", json={...})
    assert response.status_code == 201
```

## 结果

### 收益
✅ **可测试性**: 领域层可独立测试，无需数据库  
✅ **可替换性**: 基础设施组件可热插拔  
✅ **清晰边界**: 每层职责明确，降低耦合  
✅ **技术无关**: 领域模型不受框架束缚  
✅ **事件一致性**: Outbox 模式保证最终一致性  

### 权衡
⚠️ **学习曲线**: 需要理解 DDD 和六边形架构  
⚠️ **初期复杂**: 小项目可能过度设计  
⚠️ **文件数量**: 分层导致文件较多  

## 参考资料
- [Hexagonal Architecture](https://alistair.cockburn.us/hexagonal-architecture/) - Alistair Cockburn
- [Domain-Driven Design](https://www.domainlanguage.com/ddd/) - Eric Evans
- [Implementing Domain-Driven Design](https://vaughnvernon.com/) - Vaughn Vernon
- [Transactional Outbox Pattern](https://microservices.io/patterns/data/transactional-outbox.html)
