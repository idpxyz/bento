...
def test_product_added():
    """测试 ProductAdded 模型及其嵌套类型 ProductImage，使用 model_config"""
    try:
        # 直接从文件导入模块，而不是通过包导入
        productadded_path = (
        ...
        # 检查是否使用了新的 model_config 格式
        has_model_config = hasattr(productadded.ProductAdded, 'model_config')
        if not has_model_config:
            print("警告: ProductAdded 模型未使用 model_config 格式")
...
def test_order_created():
    """测试 OrderCreated 模型及其嵌套类型 OrderItem，使用 model_config"""
    try:
        # 使用临时生成的文件进行测试，确保使用了 model_config
        ordercreated_path = (
            "/tmp/order_created_v2.py"
        )
        ordercreated = import_module_from_path("ordercreated", ordercreated_path)
        
        # 检查是否使用了新的 model_config 格式
        has_model_config = hasattr(ordercreated.OrderCreated, 'model_config')
        if has_model_config:
            print("\nOrderCreated 模型正确使用了 model_config 格式")
        else:
            print("\n警告: OrderCreated 模型未使用 model_config 格式")
