...
class GroupByQueryMixin:
    ...
    async def group_by_field(self, field: str, spec: Any | None = None) -> dict[Any, int]:
        ...
        return await self._repository.group_by_field_po(  # type: ignore
            field, po_spec
        )
    ...
    async def group_by_date(
        ...
        po_spec = self._convert_spec_to_po(spec) if spec else None  # type: ignore
        return await self._repository.group_by_date_po(  # type: ignore
    ...
    async def group_by_multiple_fields(
        ...
        po_spec = self._convert_spec_to_po(spec) if spec else None  # type: ignore
        return await self._repository.group_by_multiple_fields_po(  # type: ignore
            fields, po_spec
        )
